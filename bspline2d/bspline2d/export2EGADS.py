...
import bspline2d, bspline2d_lib

def export(Ps, filename):
    oml0 = bspline2d.bspline2d()
    ...
    ms = bspline2d_lib.getsurfacesizes(oml0.nsurf, oml0.nedge, oml0.ngroup, oml0.surf_edge, oml0.edge_group, oml0.group_m)
    for i in range(oml0.nedge):
        ...
        file.write(str(k)+' ')
        file.write(str(m)+'\n')
        d = bspline2d_lib.extractd(group+1,oml0.ngroup,oml0.nD,k+m,oml0.group_k,oml0.group_m,oml0.group_d)
        for j in range(d.shape[0]):
            file.write(str(d[j])+' ')
        file.write('\n')
        C = bspline2d_lib.getsurfacep(surf+1, oml0.nC, ms[surf,0], ms[surf,1], oml0.nsurf, oml0.nedge, oml0.ngroup, oml0.nvert, oml0.surf_vert, oml0.surf_edge, oml0.edge_group, oml0.group_m, oml0.surf_index_C, oml0.edge_index_C, oml0.C)
        if edge0==0:
            if edge1==0:
    ...
    for i in range(oml0.nsurf):
        ...
        file.write(str(mv)+' ')
        file.write('\n')    
        d = bspline2d_lib.extractd(ugroup+1,oml0.ngroup,oml0.nD,ku+mu,oml0.group_k,oml0.group_m,oml0.group_d)
        for j in range(d.shape[0]):
            file.write(str(d[j])+' ')
        file.write('\n') 
        d = bspline2d_lib.extractd(vgroup+1,oml0.ngroup,oml0.nD,kv+mv,oml0.group_k,oml0.group_m,oml0.group_d)
        for j in range(d.shape[0]):
            file.write(str(d[j])+' ')
        file.write('\n')
        C = bspline2d_lib.getsurfacep(i+1, oml0.nC, ms[i,0], ms[i,1], oml0.nsurf, oml0.nedge, oml0.ngroup, oml0.nvert, oml0.surf_vert, oml0.surf_edge, oml0.edge_group, oml0.group_m, oml0.surf_index_C, oml0.edge_index_C, oml0.C)
