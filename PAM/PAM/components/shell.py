...
class Shell(Component):
    ...
    def propagateQs(self):
        ...
        shapeR0 = PAMlib.computeshape(ny, nx, (-b)/4.0, 1/4.0, r0, v['fillet'], v['shapeR0'])
        shapeT0 = PAMlib.computeshape(nz, nx, 1/4.0, 3/4.0, r0, v['fillet'], v['shapeT0'])
        shapeL0 = PAMlib.computeshape(ny, nx, 3/4.0, (4+b)/4.0, r0, v['fillet'], v['shapeL0'])
        shapeB0 = PAMlib.computeshape(nz, nx, 5/4.0, 7/4.0, r0, v['fillet'], v['shapeB0'])

        shapeR1 = PAMlib.computeshape(ny, nx, 1/4.0, (-b)/4.0, r1, v['fillet'], v['shapeR1'])
        shapeT1 = PAMlib.computeshape(nz, nx, 3/4.0, 1/4.0, r1, v['fillet'], v['shapeT1'])
        shapeL1 = PAMlib.computeshape(ny, nx, (4+b)/4.0, 3/4.0, r1, v['fillet'], v['shapeL1'])
        shapeB1 = PAMlib.computeshape(nz, nx, 7/4.0, 5/4.0, r1, v['fillet'], v['shapeB1'])
        ...
        self.Qs[0][:,:,:], Da, Di, Dj = PAMlib.computesections(-1, ny, nx, nx*ny*24, 0, r, v['offset'], chord, v['pos'], rot, shapeR0)
        self.dQs_dv[0] = scipy.sparse.csr_matrix((Da,(Di,Dj)),shape=(3*nx*ny,nQ))

        self.Qs[1][:,:,:], Da, Di, Dj = PAMlib.computesections(-1, nz, nx, nx*nz*24, 3*nx*ny, r, v['offset'], chord, v['pos'], rot, shapeT0)
        self.dQs_dv[1] = scipy.sparse.csr_matrix((Da,(Di,Dj)),shape=(3*nx*nz,nQ))

        self.Qs[2][:,:,:], Da, Di, Dj = PAMlib.computesections(-1, ny, nx, nx*ny*24, 3*nx*(ny+nz), r, v['offset'], chord, v['pos'], rot, shapeL0)
        self.dQs_dv[2] = scipy.sparse.csr_matrix((Da,(Di,Dj)),shape=(3*nx*ny,nQ))

        self.Qs[3][:,:,:], Da, Di, Dj = PAMlib.computesections(-1, ny, nx, nx*ny*24, 3*nx*(2*ny+nz), r, v['offset'], chord, v['pos'], rot, shapeL1)
        self.dQs_dv[3] = scipy.sparse.csr_matrix((Da,(Di,Dj)),shape=(3*nx*ny,nQ))

        self.Qs[4][:,:,:], Da, Di, Dj = PAMlib.computesections(-1, nz, nx, nx*nz*24, 3*nx*(3*ny+nz), r, v['offset'], chord, v['pos'], rot, shapeT1)
        self.dQs_dv[4] = scipy.sparse.csr_matrix((Da,(Di,Dj)),shape=(3*nx*nz,nQ))

        self.Qs[5][:,:,:], Da, Di, Dj = PAMlib.computesections(-1, ny, nx, nx*ny*24, 3*nx*(3*ny+2*nz), r, v['offset'], chord, v['pos'], rot, shapeR1)
        self.dQs_dv[5] = scipy.sparse.csr_matrix((Da,(Di,Dj)),shape=(3*nx*ny,nQ))

        if self.bottom==2:
            self.Qs[6][:,:,:], Da, Di, Dj = PAMlib.computesections(-1, nz, nx, nx*nz*24, 3*nx*(4*ny+2*nz), r, v['offset'], chord, v['pos'], rot, shapeB0)
            self.dQs_dv[6] = scipy.sparse.csr_matrix((Da,(Di,Dj)),shape=(3*nx*nz,nQ))

            self.Qs[7][:,:,:], Da, Di, Dj = PAMlib.computesections(-1, nz, nx, nx*nz*24, 3*nx*(4*ny+3*nz), r, v['offset'], chord, v['pos'], rot, shapeB1)
            self.dQs_dv[7] = scipy.sparse.csr_matrix((Da,(Di,Dj)),shape=(3*nx*nz,nQ))
...
if __name__ == '__main__':
    ...
    s.oml0.plot(pylab.figure(),False)
    export = PUBS.PUBSexport(s.oml0)
    name='shell'
