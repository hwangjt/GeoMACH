...
class Trussbraced(Configuration):
    ...
    def __init__(self):
        ...
        self.addComp('fuse', Body([70,10,10,20,10,10,10,50,10,30,30,10,10],[25,25,25,25,25],[15]))
        self.addComp('wing', Wing([30,10,10,50],[10,20,10,10]))
        self.addComp('strut', Wing([30],[20],opentip=True))
        self.addComp('vtail', Wing([40,10,10,10],[30,30,10],half=True))
        self.addComp('htail', Wing([20],[30]))
        ...
        self.addComp('vtailfuse', HalfInterface(self.comps, 'vtail', 0, 'fuse', 1, [0,8], [0,12]))
        self.addComp('htailvtail', FullInterface(self.comps, 'htail', 0, 'vtail', 0, [0,2], [2,1]))

        self.assembleComponents()

        self.comps['htailvtail'].setC1('surf', 0, j=0, v=0, val=False)
        self.comps['htailvtail'].setC1('surf', 0, j=-1, v=-1, val=False)
        #self.comps['htailvtail'].setC1('surf', 0, i=0, u=0, val=False)
        self.comps['htailvtail'].setC1('edge', 0, j=0, v=0)
        self.comps['htailvtail'].setC1('edge', 0, j=-1, v=-1)
        #self.comps['htailvtail'].setC1('edge', 0, i=0, u=0)
        #self.comps['htailvtail'].setCornerC1(0, i=0, j=0, val=False)
        #self.comps['htailvtail'].setCornerC1(0, i=0, j=-1, val=False)
        self.comps['htailvtail'].extraDOFs.append([None,0])
        self.comps['htailvtail'].extraDOFs.append([None,-1])
        #self.comps['htailvtail'].extraDOFs.append([0,None])
        #self.comps['htailvtail'].extraDOFs.append([0,0])
        #self.comps['htailvtail'].extraDOFs.append([0,-1])
        ...
        #print 'Q',c['vtail'].Qs[0][0,:]
        #print 'Q',c['vtail'].Qs[0][-1,:]
        print 'Q',c['htailvtail'].Qs[0][0,:]
        print 'Q',c['htailvtail'].Qs[0][:,0]
        print 'Q',c['htailvtail'].Qs[0][:,-1]
        print 'Q',c['vtail'].Qs[0][0,:]
        #print 'Q',c['htailvtail'].Qs[0][:,-1]
        #print 'N',c['vtail'].Ns[0][0,:]
        #print 'N',c['vtail'].Ns[0][-1,:]
        print 'N',c['htailvtail'].Ns[0][0,:]
        print 'N',c['htailvtail'].Ns[0][:,0]
        print 'N',c['htailvtail'].Ns[0][:,-1]
        print 'N',c['vtail'].Ns[0][0,:]
        #print 'N',c['htailvtail'].Ns[0][:,-1]
