subroutine computeProjection(surf,nu,nv,nD,nT,nC,nP0u,nP0v,nP,nsurf,nedge,& 
           ngroup,nvert,surf_vert,surf_edge,edge_group,group_k,group_m,group_n,&
           group_d,surf_index_P,edge_index_P,surf_index_C,edge_index_C,T,C,P0,&
           P,minP,minu,minv)
  ...
  !Input
  integer, intent(in) ::  surf,nu,nv,nD,nT,nC,nP0u,nP0v,nP,nsurf,nedge,&
                          ngroup,nvert
  ...
  !Output
  double precision, intent(out) ::  minP(nP0u,nP0v,3), minu(nP0u,nP0v), &
                                    minv(nP0u,nP0v)
  ...
  call getSurfaceP(surf, nP, nu, nv, nsurf, nedge, ngroup, nvert, surf_vert,&
       surf_edge, edge_group, group_n, surf_index_P, edge_index_P, P, bufferP)
  do iP0u=1,nP0u
     do iP0v=1,nP0v
        mind = 1e10
        do u=1,nu
        ...
        x(2) = bufferT(u0,v0,2)
        do counter=0,100
           call computePt(surf,0,0,nD,nC,nsurf,nedge,ngroup,nvert,x(1),x(2),&
                surf_vert,surf_edge,edge_group,group_k,group_m,group_n,group_d,&
                surf_index_P,edge_index_P,surf_index_C,edge_index_C,C,Pc)
           call computePt(surf,1,0,nD,nC,nsurf,nedge,ngroup,nvert,x(1),x(2),&
                surf_vert,surf_edge,edge_group,group_k,group_m,group_n,group_d,&
                surf_index_P,edge_index_P,surf_index_C,edge_index_C,C,Pu)
           call computePt(surf,0,1,nD,nC,nsurf,nedge,ngroup,nvert,x(1),x(2),&
                surf_vert,surf_edge,edge_group,group_k,group_m,group_n,group_d,&
                surf_index_P,edge_index_P,surf_index_C,edge_index_C,C,Pv)
           call computePt(surf,2,0,nD,nC,nsurf,nedge,ngroup,nvert,x(1),x(2),&
                surf_vert,surf_edge,edge_group,group_k,group_m,group_n,group_d,&
                surf_index_P,edge_index_P,surf_index_C,edge_index_C,C,Puu)
           call computePt(surf,1,1,nD,nC,nsurf,nedge,ngroup,nvert,x(1),x(2),&
                surf_vert,surf_edge,edge_group,group_k,group_m,group_n,group_d,&
                surf_index_P,edge_index_P,surf_index_C,edge_index_C,C,Puv)
           call computePt(surf,0,2,nD,nC,nsurf,nedge,ngroup,nvert,x(1),x(2),&
                surf_vert,surf_edge,edge_group,group_k,group_m,group_n,group_d,&
                surf_index_P,edge_index_P,surf_index_C,edge_index_C,C,Pvv)
        ...
        if (d .lt. ((Pc(1)-P0(iP0u,iP0v,1))**2 + (Pc(2)-P0(iP0u,iP0v,2))**2 + & 
           (Pc(3)-P0(iP0u,iP0v,3))**2)**0.5) then
...
subroutine computePt(surf,uder,vder,nD,nC,nsurf,nedge,ngroup,nvert,u,v,& 
           surf_vert,surf_edge,edge_group,group_k,group_m,group_n,group_d,& 
           surf_index_P,edge_index_P,surf_index_C,edge_index_C,C,P)
  ...
  allocate(Bi(nB))
  allocate(Bj(nB))
  call computeDerivativeSingle(surf, uder, vder, nB, nD, nsurf, nedge, ngroup, &
       nvert, t, surf_vert, surf_edge, edge_group, group_k, group_m, group_n, &
       group_d, surf_index_P, edge_index_P, surf_index_C, edge_index_C, Ba, & 
       Bi, Bj)
  P(:) = 0.0
  do iB=1,nB
     do k=1,3
