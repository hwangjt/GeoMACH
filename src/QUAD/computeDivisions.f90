subroutine computeDivisions(nvert0, nedge, nvert, maxL, &
...
end subroutine computeDivisions




subroutine countDivisions(nvert, nedge, maxL, edgeLengths, &
     verts, edges, ndiv)
  ...
  do iedge=1,nedge
     ...
     Lx = Lx0*(1-v(2)) + Lx1*v(2)
     Ly = Ly0*(1-v(1)) + Ly1*v(1)
     ndiv = ndiv + floor(wtdDist(Lx,Ly,v1,v2)/maxL)
  end do

end subroutine countDivisions
