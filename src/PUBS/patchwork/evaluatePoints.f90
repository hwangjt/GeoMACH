subroutine evaluatePoint(surf,uder,vder,ku,kv,mu,mv,nvar,nD,nC,nsurf,nedge,ngroup,nvert,u,v,& 
  ...
  !f2py intent(in) surf,uder,vder,ku,kv,mu,mv,nvar,nD,nC,nsurf,nedge,ngroup,nvert,u,v,surf_vert,surf_edge,edge_group,group_d,surf_index_C,edge_index_C,knot_index,C
  !f2py intent(out) P
  ...
  !f2py depend(nC,nvar) C
  !f2py depend(nvar) P

  !Input
  integer, intent(in) ::  surf,uder,vder,ku,kv,mu,mv,nvar,nD,nC,nsurf,nedge,ngroup,nvert
  ...
  integer, intent(in) ::  surf_index_C(nsurf,2), edge_index_C(nedge,2), &
                          knot_index(ngroup,2)
  double precision, intent(in) ::  C(nC,nvar)

  !Output
  double precision, intent(out) ::  P(nvar)

  !Working
  integer iB, nB, k
  double precision t(2)
  double precision Ba(ku*kv)
  integer Bi(ku*kv), Bj(ku*kv)

  nB = ku*kv
  t(1) = u
  t(2) = v
  call evaluateBasis1(surf, uder, vder, ku, kv, mu, mv, ku+mu, &
...
end subroutine evaluatePoint



subroutine evaluateBasis1(surf, uder, vder, ku, kv, mu, mv, kpmu, &
  ...
  do k1=1,ku
     do k2=1,kv
        call getIndex(surf, i0+k1, j0+k2, mu, mv, nsurf, nedge, nvert, &
...
end subroutine evaluateBasis1



subroutine evaluateBasisSurface(surf, uder, vder, nB, nT, nD, nsurf, nedge,& 
  ...
  allocate(Bu(ku))
  allocate(Bv(kv))
  call getSurfaceT(surf, nu, nv, nT, nsurf, nedge, surf_edge, surf_index_P,&
...
end subroutine evaluateBasisSurface



subroutine evaluateBnnz(nP, nsurf, nedge, ngroup, surf_edge, edge_group, &
     group_k, s, nB)
...
end subroutine evaluateBnnz



subroutine evaluateBases(uder, vder, nP, nB, nD, nsurf, nedge, ngroup, nvert,&
